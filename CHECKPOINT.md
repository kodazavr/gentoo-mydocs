...
пересборок. ccache оставлен глобально (вариант A, 2026-09-12); возврат к
вопросу — после контрольного замера hit rate через 4–8 недель. Сужение USE
завершено 2026-09-12: `sound-server`, `screencast`, `lto`, `gles2` перенесены
в `package.use`, шесть остальных кандидатов подтверждены глобальной
политикой, пересборок нет. Пакетная политика закрыта 2026-09-12:
`video_cards_i915` у mesa и все `llvm_slot_*`-правила удалены владельцем,
политика ядра сверена и синхронизирована, resolver-план без отдельных
переустановок. Записи noctalia в world-файле приведены к одной;
savedconfig-файл linux-firmware и `*.bak` осознанно оставлены владельцем;
остался календарный замер ccache; живая система не изменяется без
отдельного согласования.

---

## Активная задача: по-пакетный аудит USE-флагов установленного софта
...
`USE` удалены семь флагов без потребителей. Основная работа завершена 2026-09-12. Открыт по-пакетный аудит USE
установленного софта батчами по 5 пакетов; источник флагов — VDB `IUSE` и
ebuild (equery может показывать неполный список). Батч 1 (mesa, firefox,
pipewire, ffmpeg, niri) разобран и закрыт 2026-09-12: мёртвые токены
(`liblc3`, `-suid`, `-test`, `-nautilus`, два `-iptables`) удалены,
`-gnutls`/`-wifi`/`-jpegxl` закреплены; resolver показывает только две
ожидаемые переустановки. Батчи 2 и 2b (все 11 модулей Qt) разобраны
2026-09-13: обязательных правок нет; `-jit`/`-orc` — от hardened-профиля;
глобальный `wifi` удалён; `libproxy` у qtbase выключен (прокси нет, пакет
держит glib-networking). Батч 3 (mpv, networkmanager, docker, podman,
libvirt) закрыт 2026-09-13: NM почищен (`-modemmanager -ppp -bluetooth`;
конфликт `bluetooth? ( modemmanager )` вскрыт resolver'ом и решён),
libvirt ждёт включения `virtiofsd` правилом владельца. Батч 4 (qemu,
spice-gtk, virt-manager, libvirt) закрыт 2026-09-13: у qemu оставлен один
таргет x86_64 (ARM и i386 убраны, 32-битные гости покрываются x86_64 с
32-битными CPU-моделями), user-mode убран как не-песочница, `-X` был no-op
(форс профиля), spice-gtk без sasl, libvirt с virtiofsd (+ отдельный пакет).
Накоплено 7 осознанных переустановок и 1 новый пакет — при ближайшем
`emerge -avuDN @world` применятся; после него depclean-кандидаты:
modemmanager, ppp. Очередь аудита USE: iproute2, dracut, sbctl и прочие
системные из 10/20-файлов.

### Быстрый старт следующей сессии

Не перечитывать все документы: состояние и решения зафиксированы здесь и в
аудитах `2026-09-12-make-conf-policy.md`, `2026-09-13-use-flags-batch2-qt.md`,
`2026-09-13-use-flags-batch3.md`, `2026-09-13-use-flags-batch4-virtualization.md`
(каталог `systems/asus-b5402/audits/`). Задокументированные проверки не
перепроверять заново без изменений системы; новый resolver-план нужен только
после мирового обновления. Методика серии: список флагов брать из VDB `IUSE`
(`equery uses` неполон), перед выключением флага смотреть `REQUIRED_USE`,
помнить про `use.force` профиля (сильнее package.use). Продолжение — батч
системных пакетов (iproute2, dracut, sbctl, linux-firmware, intel-microcode). Ожидают календаря и внешних
событий: замер ccache — сначала
`CCACHE_DIR=/var/tmp/ccache ccache --zero-stats`, затем 4–8 недель обычной
работы и снятие hit rate; плановый
переход на LLVM 23 — когда ebuild'ы потребителей объявят `llvm_slot_23`
(сигнал: дефолты сами перевернутся на 23). Осознанно отложены владельцем:
файл `savedconfig/sys-kernel/linux-firmware-20260810` при выключенном USE и
`*.bak` рядом с ним. Живая система не изменяется без отдельного
согласования.
...
| Дата | Событие |
|------|---------|
| 2026-09-12 | Подтверждена политика `make.conf` (`-O3`/ThinLTO, `-mno-*`, `-l10`); из глобального USE удалены 7 флагов без потребителей, resolver-план без пересборок; ccache оставлен глобально (вариант A) с контрольным замером через 4–8 недель; `sound-server`, `screencast`, `lto`, `gles2` перенесены в `package.use` без пересборок; зафиксированы кандидаты: world-запись `noctalia-local`, `video_cards_i915` у mesa |
| 2026-09-12 | Пакетная политика: `video_cards_i915` у mesa и все `llvm_slot_*`-правила удалены владельцем; потребители резолвятся в 22 (слот 23 ebuild'ами не поддерживается — watchpoint для будущего перехода); политика ядра сверена (`-generic` устарел, `modules-compress`/`linux-firmware`/`intel-microcode` зафиксированы); world-запись `noctalia-local` заменена, остались дубль и пин `::guru` |
| 2026-09-12 | World-файл приведён к одной записи `gui-apps/noctalia`; savedconfig-файл linux-firmware и `*.bak` осознанно оставлены; задача пакетной политики закрыта |
| 2026-09-13 | Аудит USE Qt (батчи 2 и 2b, 11 модулей): правок не потребовалось; зафиксированы `-jit`/`-orc` от hardened-профиля, дефолт `+X` qtbase под глобальным `-X`, слоты qttools ≤21; `wifi` удалён из глобального USE; libproxy у qtbase признан лишним (решение владельца) |
| 2026-09-13 | Батч 3: NM — выключены `modemmanager`, `ppp` и `bluetooth` (конфликт `bluetooth? ( modemmanager )` вскрыт resolver'ом); libvirt — решено включить `virtiofsd`; depclean-кандидаты после пересборки NM: modemmanager, ppp |
| 2026-09-13 | Батч 4 (виртуализация): qemu — таргеты сокращены до x86_64 (ARM/i386/user-mode убраны; user-mode зафиксирован как не-песочница), `-X` удалён как no-op (форс профиля), io-uring подтверждён ядром; spice-gtk — `-sasl`; libvirt — `virtiofsd` с отдельным пакетом; resolver: 10 апгрейдов + 7 осознанных переустановок + 1 новый |
