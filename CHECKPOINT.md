...
`USE` удалены семь флагов без потребителей. Основная работа завершена 2026-09-12. Ожидают календаря и внешних событий:
замер ccache — сначала `CCACHE_DIR=/var/tmp/ccache ccache --zero-stats`,
затем 4–8 недель обычной работы и снятие hit rate; плановый переход на
LLVM 23 — когда ebuild'ы потребителей объявят `llvm_slot_23` (сигнал:
дефолты сами перевернутся на 23). Осознанно отложены владельцем: файл
