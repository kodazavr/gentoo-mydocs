...
| Параметр | Значение |
|----------|----------|
| Дата последнего аудита | Нет актуального; источник аудита 2026-06-13 отсутствует |
...
| Последнее записанное состояние системы | Gentoo Linux, ядро `7.1.5-bdsm`, systemd-boot, Secure Boot + TPM2; в этой сессии не проверялось |
...
Живая система в этой сессии не проверялась. Прежний аудит и подготовленный вне
Git каталог `configs/` отсутствуют и признаны устаревшими источниками. Старые
утверждения ниже сохраняют исторический контекст, но не подтверждают текущее
состояние машины.

Активная работа относится к структуре публичной документации и не изменяет
живую систему.

---

## Активная задача: публичная документация и системный журнал
